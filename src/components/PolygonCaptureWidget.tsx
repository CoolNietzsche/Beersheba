...
export default function PolygonCaptureWidget({
  ...
  const updateMap = useCallback((pts: [number, number][]) => {
    ...
    pts.forEach(([lng, lat], i) => {
      L.circleMarker([lat, lng], { radius: 6, color: T.color.coffee, fillColor: T.color.gold, fillOpacity: 1 })
  ...
  const drawPolygon = (coords: [number, number][], map: L.Map) => {
    ...
    polyLayer.current = L.polygon(latlngs, {
      color: T.color.gold, fillColor: T.color.gold, fillOpacity: 0.2, weight: 2,
    }).addTo(map);
  ...
  return (
    <div style={{ ...CS.card, padding: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <span style={{ fontFamily: T.font.display, fontSize: "1.1rem", fontWeight: 500, color: T.color.ink }}>
          Farm Boundary
        </span>
        <span style={{
          fontSize: "0.58rem", fontFamily: T.font.mono,
          background: online ? T.color.forestLight : T.color.linen,
          color: online ? T.color.forest : T.color.coffee,
          padding: "3px 10px", borderRadius: "20px", fontWeight: 650,
          border: `1px solid ${online ? "rgba(27,77,53,0.15)" : T.color.border}`,
        }}>
        ...
        {(["pin", "walk", "import"] as Mode[]).map(m => (
          <button key={m} onClick={() => { setTab(m); clearPoints(); }}
            style={{
              ...
              fontFamily: T.font.sans, fontSize: 13, cursor: "pointer",
              background: tab === m ? T.color.forest : T.color.linen,
              color: tab === m ? "#fff" : T.color.textMuted,
              fontWeight: tab === m ? 600 : 500,
              borderWidth: "1px",
              borderStyle: "solid",
              borderColor: tab === m ? T.color.forest : T.color.border,
            }}>
      ...
      {/* Instructions */}
      <div style={{ fontSize: 12, color: T.color.textMuted, marginBottom: 12, fontFamily: T.font.sans, lineHeight: 1.5 }}>
      ...
      {/* Map */}
      <div ref={mapRef} style={{ height: 300, borderRadius: T.radius.sm, marginBottom: 12, border: `1px solid ${T.color.border}` }} />
      ...
      {tab === "walk" && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <button onClick={startWalk} disabled={walking}
            style={{
              flex: 1, padding: "10px 0", borderRadius: T.radius.sm, border: "none",
              background: walking ? T.color.linen : T.color.forestLight, color: T.color.forest,
              fontFamily: T.font.sans, fontSize: 14, cursor: walking ? "not-allowed" : "pointer",
              fontWeight: 600,
            }}>
          ...
          <button onClick={stopWalk} disabled={!walking}
            style={{
              flex: 1, padding: "10px 0", borderRadius: T.radius.sm, border: "none",
              background: !walking ? T.color.linen : T.color.red, color: "#fff",
              fontFamily: T.font.sans, fontSize: 14, cursor: !walking ? "not-allowed" : "pointer",
              fontWeight: 600,
            }}>
            ■ Stop & Close
      ...
      {tab === "import" && (
        <div style={{ marginBottom: 12 }}>
          <input type="file" accept=".geojson,.json,.kml"
            ...
            style={{ color: T.color.ink, fontFamily: T.font.sans, fontSize: 13 }} />
        </div>
      )}
      ...
      {areaHa !== null && (
        <div style={{
          background: T.color.forestLight, borderRadius: T.radius.sm, padding: "8px 14px",
          marginBottom: 12, fontFamily: T.font.mono, fontSize: 13, color: T.color.forest,
          border: `1px solid rgba(27,77,53,0.15)`, fontWeight: 600,
        }}>
      ...
      {/* Status message */}
      {status && (
        <div style={{ fontSize: 12, color: T.color.coffee, marginBottom: 12, fontFamily: T.font.sans, fontWeight: 500 }}>
          {status}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {polygon && (
          <button onClick={handleSave} disabled={saving}
            style={{
              flex: 2, padding: "12px 0", borderRadius: T.radius.sm, border: "none",
              background: T.color.forest, color: "#fff", fontWeight: 600,
        ...
        {canInherit && mode === "lot" && (
          <button onClick={handleInherit} disabled={saving}
            style={{
              flex: 1, padding: "12px 0", borderRadius: T.radius.sm, border: `1px solid ${T.color.forest}`,
              background: "transparent", color: T.color.forest,
              fontFamily: T.font.sans, fontSize: 13, cursor: "pointer", fontWeight: 600,
            }}>
            ↙ From Farm
          </button>
        )}
        {points.length > 0 && (
          <button onClick={clearPoints}
            style={{
              flex: 1, padding: "12px 0", borderRadius: T.radius.sm, border: `1px solid ${T.color.border}`,
